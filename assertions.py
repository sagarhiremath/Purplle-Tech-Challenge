"""
Example assertions your API must pass (challenge acceptance checks).

Run with API up:
    python scripts/run_assertions.py

Requires sample_events.jsonl in data/ (created by pipeline/run.sh).
"""
...
ALL_ASSERTIONS = [
    assert_metrics_endpoint_returns_json,
    assert_funnel_has_four_stages,
    assert_health_reports_store_status,
    assert_heatmap_normalized_scores,
    assert_anomalies_have_severity,
    assert_zero_traffic_store_safe,
    assert_staff_excluded_from_conversion_rate,
    assert_funnel_drop_off_non_negative,
]

INGEST_ASSERTIONS = [
    assert_ingest_accepts_batch,
    assert_ingest_is_idempotent,
]
